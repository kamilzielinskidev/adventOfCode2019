...
const rawWirePathToWirePath = raw =>
  raw.split(",").map(value => {
    const separatedVal = value.match(/[A-Z]|\d+/g);
    return {
      direction: String(separatedVal[0]),
      value: Number(separatedVal[1])
    };
  });

const firstWirePath = rawWirePathToWirePath(firstWirePathRaw);
const secondWirePath = rawWirePathToWirePath(secondWirePathRaw);

// generatePath = wirePath => {
//   const path = wirePath.reduce((prevVal, currVal) => {
//     switch (currVal.direction) {
//       case "R":
//         return `${prevVal} h ${currVal.value}`;
//       case "L":
//         return `${prevVal} h ${-currVal.value}`;
//       case "U":
//         return `${prevVal} v ${-currVal.value}`;
//       case "D":
//         return `${prevVal} v ${currVal.value}`;
//     }
//   }, "M 500 500");
...
const findConnections = wirePath => {
  const connections = [];
  let currentPosition = { x1: 0, y1: 0 };
  wirePath.forEach(coords => {
    switch (coords.direction) {
      case "R":
        connections.push({ ...currentPosition, ...{ x2: (currentPosition.x1 += coords.value), y2: currentPosition.y1 } });
        break;
      case "L":
        connections.push({ ...currentPosition, ...{ x2: (currentPosition.x1 -= coords.value), y2: currentPosition.y1 } });
        break;
      case "U":
        connections.push({ ...currentPosition, ...{ x2: currentPosition.x1, y2: (currentPosition.y1 += coords.value) } });
        break;
      case "D":
        connections.push({ ...currentPosition, ...{ x2: currentPosition.x1, y2: (currentPosition.y1 -= coords.value) } });
        break;
...
const firstWireConnections = findConnections(firstWirePath);
const secondWireConnections = findConnections(secondWirePath);

secondWireConnections.forEach(({ x1: SWx1, y1: SWy1, x2: SWx2, y2: SWy2 }) => {
  firstWireConnections.forEach(({ x1: FWx1, y1: FWy1, x2: FWx2, y2: FWy2 }) => {
    if (FWx1 === FWx2) {
      if (SWy1 === SWy2) {
        if (SWx1 > FWx1 && SWx1 < FWx2) {
          console.log("cross!");
        }
      }
    }
    ...
    if (FWy1 === FWy2) {
      if (SWx1 === SWx2) {
        if (SWy1 > FWy1 && SWy1 < FWy2) {
          console.log("cross!");
        }
      }
    }
    // if (SWx1 < FWx1 && SWx2 > FWx1 && SWy1 < FWy2 && SWy1 > FWy1) {
    //   closestIntersection =
    //     closestIntersection === null || closestIntersection > Math.abs(FWx1) + Math.abs(SWy1) ? Math.abs(FWx1) + Math.abs(SWy1) : closestIntersection;
    // }
    // if (SWx1 < FWx1 && SWx2 > FWx1 && SWy1 > FWy2 && SWy1 < FWy1) {
    //   closestIntersection =
    //     closestIntersection === null || closestIntersection > Math.abs(FWx1) + Math.abs(SWy1) ? Math.abs(FWx1) + Math.abs(SWy1) : closestIntersection;
    ...
    //     closestIntersection === null || closestIntersection > Math.abs(FWx1) + Math.abs(SWy1) ? Math.abs(FWx1) + Math.abs(SWy1) : closestIntersection;
    // }
    // if (SWx1 > FWx1 && SWx1 < FWx2 && SWy1 < FWy1 && SWy2 > FWy1) {
    //   closestIntersection =
    //     closestIntersection === null || closestIntersection > Math.abs(FWy1) + Math.abs(SWx1) ? Math.abs(FWy1) + Math.abs(SWx1) : closestIntersection;
    // }
    // if (SWx1 < FWx1 && SWx1 > FWx2 && SWy1 < FWy1 && SWy2 > FWy1) {
    //   closestIntersection =
    //     closestIntersection === null || closestIntersection > Math.abs(FWy1) + Math.abs(SWx1) ? Math.abs(FWy1) + Math.abs(SWx1) : closestIntersection;
